...
Nombre_benchmark = '2-Fullins_3'#"jean"#"anna"#"DSJC250-5"#"DSJC500-5"#"le450_15c"#"le450_25c"#'flat300_28'#"myciel3"#"myciel4"#
...
def BusquedaLocal_CUDA(nuevo_individuos, probabilidades,Aristmono,MatrizAdjacencia, H,numNodos,tam_pobla,rng_states):
    ...
    individuoCuda = cuda.shared.array(shape= (numColores,NODOS), dtype= int32)
    probabilidadCuda = cuda.shared.array(shape = numColores, dtype = float32)
    MatrizAdjCuda = cuda.shared.array(shape=(NODOS,NODOS), dtype=int32)
    ...
    if id < tam_pobla:
        ...
        """if id == 0:
            for i in range(numColores):
                #print(probabilidadCuda[i])
                for j in range(numNodos):
                    if individuoCuda[i][j] == 1:
                        print(i,":",j+1)"""

        #metropolis_gpu(MatrizAdjCuda, individuoCuda, probabilidadCuda,AristasCuda,numColores,numNodos,rng_states,id)  # regresa al individuo después de realzar la búsqueda local
        Escalando_gpu(MatrizAdjCuda, individuoCuda, probabilidadCuda,AristasCuda,numColores,numNodos,rng_states,id)  # regresa al individuo después de realzar la búsqueda local
        """if id == 0:
            for i in range(numColores):
                #print(probabilidadCuda[i])
                for j in range(numNodos):
                    if nuevo_individuos[0][i][j] == 1:
                        print(i,":",j+1)"""
...
def imprimeResultados(best,best_ind,tiempo,archivo):
    print('\n\nmejor número de aristas monocromáticas: ')
    print(str(best))
    print('mejor individuo: ')
    print(best_ind)
    print("%s seconds" % (time.time() - tiempo))
    archivo.write(str(time.time() - tiempo) + '\t' + str(best[0]) + '\n')
